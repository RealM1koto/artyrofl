...
# Пути к файлам в папке с исполняемым файлом
$script:songPath = Join-Path $script:currentDir "song.mp3"
$script:imgPath = Join-Path $script:currentDir "bug.jpg"

# Проверяем существование файлов
if (-not (Test-Path $script:songPath)) {
    Write-Error "Файл song.mp3 не найден в папке: $script:currentDir"
    exit 1
}

if (-not (Test-Path $script:imgPath)) {
    Write-Error "Файл bug.jpg не найден в папке: $script:currentDir"
    exit 1
}
...
$script:REST = [System.Text.Encoding]::UTF8.GetString([Convert]::FromBase64String('0KfQnQ=='))
$script:NREST = [System.Text.Encoding]::UTF8.GetString([Convert]::FromBase64String('0J3QlSDQp9Cd'))

# 8 ЧН + 2 НЕ ЧН = 10 секторов
$script:lbl = @(
